...
import { uint8Max, uint16Max, kIngameSaveSlot } from "../core/game_constants"
import { gameDebugLog } from './game-debug'
import { getRuntimeRegistryState } from './game-runtime-data'
import { getGamePgeState, getGameSessionState, getGameWorldState } from './game-state'
...
const formatOpcodeSnapshotDiff = (before: OpcodeDebugSnapshot | null | undefined, after: OpcodeDebugSnapshot | null | undefined) => {
	if (!after) return ''
	if (!before) return Object.entries(after).map(([key, value]) => `${key}=${value}`).join(' ')
	const keys = new Set([...Object.keys(before), ...Object.keys(after)])
	const changes: string[] = []
	keys.forEach((key) => {
		const beforeValue = before[key]
		const afterValue = after[key]
		if (beforeValue !== afterValue) changes.push(`${key}:${beforeValue}->${afterValue}`)
	})
	return changes.join(' ')
}
...
const opcodeCreditsSnapshot = (_args: PgeOpcodeArgs, game: Game) => ({
	credits: getGameWorldState(game).credits
})
...
const opcodeCutsceneSnapshot = (_args: PgeOpcodeArgs, game: Game) => {
	...
	return { cutsceneId, deathCutsceneCounter: world.deathCutsceneCounter }
}
...
export const instrumentOpcodeTable = (handlers: (PgeOpcodeHandler | null)[]) =>
	handlers.map((handler, index) => {
		...
		if (!handler || !metadata) return handler
