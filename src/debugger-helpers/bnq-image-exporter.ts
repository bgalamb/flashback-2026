...
class BnqImageExporter {
    private static readonly TILE_SIZE = 8
    private static readonly TILE_FRAME = 1
    private static readonly TILE_STRIDE = BnqImageExporter.TILE_SIZE + BnqImageExporter.TILE_FRAME * 2

    private _bnq: Uint8Array
    private _palette: RgbColor[]
    private _bankCache: { [key: string]: Uint8Array } = {}

    constructor(bnq: Uint8Array, palette?: RgbColor[]) {
        this._bnq = bnq
        this._palette = palette && palette.length >= 16 ? palette.slice(0, 16) : BnqImageExporter.defaultPalette()
    }

    static exportTilesImage(bnqPath: string, outputPath: string, palettePath?: string, paletteSlot: number = 0, tilesPerRow: number = 16) {
        const fs = require("fs")
        const bnq = new Uint8Array(fs.readFileSync(bnqPath))
        const resolvedPalettePath = palettePath || BnqImageExporter.resolvePalettePath(bnqPath)
        const palette = resolvedPalettePath ? BnqImageExporter.loadPalette(resolvedPalettePath, paletteSlot) : undefined
        const exporter = new BnqImageExporter(bnq, palette)
    ...
    buildImage(tilesPerRow: number = 16) {
        ...
        const width = safeTilesPerRow * BnqImageExporter.TILE_STRIDE
        const rows = Math.max(1, Math.ceil(tileCount / safeTilesPerRow))
        const height = rows * BnqImageExporter.TILE_STRIDE
        ...
        for (let entry = 0; entry < entryCount; ++entry) {
            ...
            for (let offset = 0; offset + 32 <= bankData.length; offset += 32) {
                const x = (tileIndex % safeTilesPerRow) * BnqImageExporter.TILE_STRIDE + BnqImageExporter.TILE_FRAME
                const y = ((tileIndex / safeTilesPerRow) >> 0) * BnqImageExporter.TILE_STRIDE + BnqImageExporter.TILE_FRAME
    ...
    private static resolvePalettePath(bnqPath: string) {
        const fs = require("fs")
        const path = require("path")
        const ext = path.extname(bnqPath)
        const base = bnqPath.slice(0, bnqPath.length - ext.length)
        const candidates = [
            `${base}.pal`,
            `${base}.PAL`
        ]
    ...
    private static loadPalette(palPath: string, paletteSlot: number) {
        ...
        for (let i = 0; i < 16; ++i) {
            colors.push(BnqImageExporter.amigaConvertColor(READ_BE_UINT16(pal, slotOffset + i * 2), true))
        }
        return colors
    }
    ...
    private getBankDataSize(num: number) {
        let len = READ_BE_UINT16(this._bnq, num * 6 + 4)
        if (len & 0x8000) {
            len = -(len << 16 >> 16)
        }
        return len * 32
    ...
    private loadBankData(num: number) {
        ...
        const ptr = this._bnq.subarray(num * 6)
        ...
        const data = this._bnq.subarray(dataOffset)
    ...
    private drawTile(dst: Uint8Array, dstWidth: number, x: number, y: number, src: Uint8Array) {
        let srcIndex = 0
        for (let row = 0; row < BnqImageExporter.TILE_SIZE; ++row) {
            let dstIndex = (y + row) * dstWidth + x
            for (let col = 0; col < BnqImageExporter.TILE_SIZE; col += 2) {
