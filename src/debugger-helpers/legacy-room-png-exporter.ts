...
class AmigaLevelImageExporter {
    ...
    static exportRoomImage(levPath: string, mbkPath: string, palPath: string, sgdPath: string, level: number, room: number, outputPath: string, prevFrontLayer? :Uint8Array): Uint8Array {
        ...
        if (frontLayer != prevFrontLayer) {
            exporter.writeRgbPng(outputPath, frontLayer)
        }
        return frontLayer
    }
    ...
    static exportAllGameLevelRooms(dataDir: string, outputDir: string, roomFrom: number = 1, roomTo: number = 100) {
        ...
        for (let levelIndex = 0; levelIndex < _gameLevels.length; ++levelIndex) {
            ...
            for (let room = roomFrom; room <= roomTo; ++room) {
                ...
                const outputPath = path.join(levelOutputDir, `${resolved.baseName}-room${room}.png`)
    ...
    static exportRoomLayerArtifacts(
        ...
        exporter.writeIndexedPng(`${outputPrefix}.pixeldata.png`, frontLayer)
        exporter.writeIndexedPngLayerGroup(`${outputPrefix}-backlayer.png`, frontLayer, "back")
        exporter.writeIndexedPngLayerGroup(`${outputPrefix}-frontlayer.png`, frontLayer, "front")
    }
    ...
    private static resolveDataFileInDirectories(baseName: string, ext: string, directories: string[]): string | null {
        const fs = require('fs')
        const path = require('path')
        for (let i = 0; i < directories.length; ++i) {
            const directory = directories[i]
            const candidates = [
                path.join(directory, `${baseName}.${ext.toLowerCase()}`),
                path.join(directory, `${baseName}.${ext.toUpperCase()}`)
            ]
    ...
    private static resolveLevelAssetPaths(dataDir: string, levelName: string) {
        ...
        const palPath = AmigaLevelImageExporter.resolveDataFileInDirectories(baseName, "pal", [
            dataDir,
            path.join(dataDir, "levels", "legacy-level-data"),
            path.join(dataDir, "levels", "legacy-level-data", "palettes")
        ])
    ...
    private writeRgbPng(outputPath: string, frontLayer: Uint8Array) {
        const fs = require('fs')
    ...
    private buildPaletteFromRgbPalette() {
        const palette: Color[] = []
        for (let i = 0; i < 256; ++i) {
            const offset = i * 3
            palette.push({
                r: this._rgbPalette[offset + 0],
                g: this._rgbPalette[offset + 1],
                b: this._rgbPalette[offset + 2]
            })
        }
        return palette
    }

    private buildLayerPalette(layerGroup: "back" | "front") {
        const palette: Color[] = []
        for (let i = 0; i < 256; ++i) {
    ...
    private copyPaletteRange(palette: Color[], dstSlot: number, srcSlot: number) {
        for (let i = 0; i < 16; ++i) {
            const srcOffset = (srcSlot * 16 + i) * 3
            palette[dstSlot * 16 + i] = {
                r: this._rgbPalette[srcOffset + 0],
                g: this._rgbPalette[srcOffset + 1],
                b: this._rgbPalette[srcOffset + 2]
            }
        }
    }
