function printUsage() {
    console.error("Usage: npx ts-node --transpile-only ./src/debugger-helpers/export-all-room-indexed-png.ts <dataDir>")
}
    ...
    for (let i = 0; i < pixelFiles.length; ++i) {
        ...
        childProcess.execFileSync(
            ...
            [
                "ts-node",
                "--transpile-only",
                "./src/debugger-helpers/export-room-indexed-png.ts",
                pixelPath,
                palettePath,
                paletteHeaderPath,
                outputPath
