...
function markScreenBlockAsDirty(layers: VideoLayerState, screen: VideoScreenState, x: number, y: number, w: number, h: number, scale: number) {
    ...
    if (bx2 > ((layers.w / SCREENBLOCK_W) >> 0) - 1) {
        bx2 = ((layers.w / SCREENBLOCK_W) >> 0) - 1
    }
    if (by1 < 0) {
        by1 = 0
    }
    if (by2 > ((layers.h / SCREENBLOCK_H) >> 0) - 1) {
        by2 = (((layers.h / SCREENBLOCK_H) >> 0) - 1) >> 0
    }
    for (; by1 <= by2; ++by1) {
        for (let i = bx1; i <= bx2; ++i) {
            screen.screenBlocks[by1 * ((layers.w / SCREENBLOCK_W) >> 0) + i] = 2
        }
    }
}
...
function requestFullRefresh(screen: VideoScreenState, layers: VideoLayerState) {
    ...
    screen.screenBlocks.fill(0, (layers.w / SCREENBLOCK_W) * (layers.h / SCREENBLOCK_H))
}
...
async function updateVideoScreen(stub: SystemStub, layers: VideoLayerState, screen: VideoScreenState) {
    if (screen.fullRefresh) {
        ...
        let index = 0
        for (j = 0; j < layers.h / SCREENBLOCK_H; ++j) {
            let nh = 0
            for (i = 0; i < layers.w / SCREENBLOCK_W; ++i) {
                if (p[i + index] !== 0) {
                    --p[i + index]
                    ++nh
                } else if (nh !== 0) {
                    const x = (i - nh) * SCREENBLOCK_W
                    stub.copyRect(x, j * SCREENBLOCK_H, nh * SCREENBLOCK_W, SCREENBLOCK_H, layers.frontLayer, layers.w)
                    nh = 0
                    ++count
                }
            }
