...
SoundProcessor.kUseNr = false;
SoundProcessor.nr = (buf /*: Int16Array*/, len /*: number*/) => {
    let prev = 0;
    for (let i = 0; i < len; ++i) {
        const vnr = buf[i] >> 1;
        buf[i] = vnr + prev;
...
SoundFxProcessor.kUseNr = false;
SoundFxProcessor.nr = (buf /*: Int16Array*/, len /*: number*/) => {
    let prev = 0;
    for (let i = 0; i < len; ++i) {
        const vnr = buf[i] >> 1;
        buf[i] = vnr + prev;
